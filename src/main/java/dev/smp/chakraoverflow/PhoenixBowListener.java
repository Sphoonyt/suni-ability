    ...
    private void phoenixFinalExplosion(Location center, Player target, Player shooter) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                // ── Outward ring — solid wall of fire, no gaps ──
                int points = (int) Math.max(64, expandRadius * 12); // ~12 points per block of circumference
                for (int i = 0; i < points; i++) {
                    double a     = i * (Math.PI * 2.0 / points);
                    double cosA  = Math.cos(a);
                    double sinA  = Math.sin(a);
                    Location rim = center.clone().add(cosA * expandRadius, 0.15, sinA * expandRadius);
                    // Stack 3 height layers so it's a thick wall not just a line on the ground
                    for (double hy = 0; hy < 1.2; hy += 0.4) {
                        Location rimH = rim.clone().add(0, hy, 0);
                        world.spawnParticle(Particle.FLAME,           rimH, 3, 0.03, 0.05, 0.03, 0);
                        world.spawnParticle(Particle.SOUL_FIRE_FLAME,  rimH, 2, 0.03, 0.04, 0.03, 0);
                    }
                    if (i % 4 == 0) world.spawnParticle(Particle.LAVA, rim, 1, 0, 0.3, 0, 0);
                }

                // ── Fire pillars rising from the ring — dense columns ──
                int pillars = (int) Math.max(12, expandRadius * 1.5);
                for (int p = 0; p < pillars; p++) {
                    double a = p * (Math.PI * 2.0 / pillars) + tick * 0.05;
                    double pillarHeight = 5.0 + expandRadius * 0.6;
                    for (double h = 0; h < pillarHeight; h += 0.18) { // tight step
                        ...
                        world.spawnParticle(Particle.FLAME,           pillar, 2, 0.04, 0.02, 0.04, 0);
                        world.spawnParticle(Particle.SOUL_FIRE_FLAME,  pillar, 1, 0.03, 0.02, 0.03, 0);
                    }
                }
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < 24; i++) { // 24 arms, tight spiral
