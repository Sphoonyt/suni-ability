...
public class PhoenixBowListener implements Listener {
    ...
    private void phoenixEruption(Location impactLoc, Player target, Player shooter) {
        ...
        // ── Phase 1: Helix tornado spawns at 15-block radius and closes in ──
        // The tornado tracks the target's current position each tick
        final double START_RADIUS = 15.0;
        final double HELIX_HEIGHT = 18.0;
        final int CLOSE_TICKS    = 80;   // ticks to close from 15 → 0
        final int ARMS            = 5;    // number of helix arms
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                if (tick >= CLOSE_TICKS) {
                    ...
                    Location finalLoc = target.isOnline() ? target.getLocation() : impactLoc;
                    phoenixFinalExplosion(finalLoc, target, shooter);
                    return;
                }

                // Radius shrinks linearly from START_RADIUS → 0
                double easeT    = tick / (double) CLOSE_TICKS;
                double radius   = START_RADIUS * (1.0 - easeT);
                // Spin speed ramps up as it closes in (like a whirlpool)
                double spinRate = 12.0 + easeT * 30.0;
                double baseAngle = tick * spinRate * (Math.PI / 180.0);

                // Track target
                Location origin = target.isOnline()
                ...
                // ── Spinning ground ring (traces the base of the tornado) ──
                for (int i = 0; i < 32; i++) {
                    double a = baseAngle + i * (Math.PI * 2.0 / 32);
                    Location gLoc = origin.clone().add(Math.cos(a) * radius, -0.4, Math.sin(a) * radius);
                    world.spawnParticle(Particle.FLAME, gLoc, 1, 0, 0, 0, 0);
                    if (i % 4 == 0) {
                        world.spawnParticle(Particle.SOUL_FIRE_FLAME, gLoc, 1, 0, 0, 0, 0);
                    }
                }

                // ── Top cap — closing crown of particles ──
                for (int i = 0; i < 16; i++) {
                    double a = baseAngle * 1.5 + i * (Math.PI * 2.0 / 16);
                    double cr = radius * 0.3;
                    Location top = origin.clone().add(Math.cos(a) * cr, HELIX_HEIGHT + 1.0, Math.sin(a) * cr);
                    world.spawnParticle(Particle.FLAME, top, 2, 0.1, 0.15, 0.1, 0.03);
                }

                // ── Ambient rising ash ──
                if (tick % 3 == 0) {
                    for (int i = 0; i < 6; i++) {
                        double ax = (Math.random() - 0.5) * radius * 2;
                        double az = (Math.random() - 0.5) * radius * 2;
                        world.spawnParticle(Particle.ASH, origin.clone().add(ax, Math.random() * 4, az),
                                1, 0, 0.05, 0, 0.01);
                    }
                }

                // ── Sound: rumble pitch rises as it closes ──
                if (tick % 6 == 0) {
                    float pitch = 0.4f + (float) easeT * 1.4f;
                    world.playSound(origin, Sound.BLOCK_FIRE_AMBIENT, 1.8f, pitch);
                    if (easeT > 0.5) {
                        world.playSound(origin, Sound.ENTITY_BLAZE_BURN, 1.2f, pitch);
                    }
                }
    ...
    // ─────────────────────────────────────────────
    //  Phase 2: Final outward explosion once tornado collapses
    // ─────────────────────────────────────────────
    ...
    private void phoenixFinalExplosion(Location center, Player target, Player shooter) {
        ...
        // Massive blind + sound to everyone nearby
        for (Player nearby : world.getPlayers()) {
        ...
        // Expanding explosion rings — 5 waves at increasing radii
        final int WAVES = 5;
        for (int wave = 0; wave < WAVES; wave++) {
            final double waveRadius = 3.0 + wave * 3.5;
            final int waveDelay = wave * 3;

            new BukkitRunnable() {
                @Override
                public void run() {
                    // Full 360° ring of flame + soul fire + lava
                    for (int i = 0; i < 72; i++) {
                        double a = i * (Math.PI * 2.0 / 72);
                        Location rim = center.clone().add(Math.cos(a) * waveRadius, 0.15, Math.sin(a) * waveRadius);
                        world.spawnParticle(Particle.FLAME, rim, 3, 0, 0.2, 0, 0.08);
                        if (i % 3 == 0) world.spawnParticle(Particle.SOUL_FIRE_FLAME, rim, 2, 0, 0.15, 0, 0.05);
                        if (i % 9 == 0) world.spawnParticle(Particle.LAVA, rim, 1, 0, 0.3, 0, 0);
                    }
                    // Vertical pillars at ring points
                    for (int p = 0; p < 12; p++) {
                        double a = p * (Math.PI * 2.0 / 12);
                        for (double h = 0; h < 6 + waveRadius * 0.5; h += 0.4) {
                            Location pillar = center.clone().add(
                                    Math.cos(a) * waveRadius * (1.0 - h / 20.0),
                                    h,
                                    Math.sin(a) * waveRadius * (1.0 - h / 20.0)
                            );
                            if (Math.random() < 0.5) world.spawnParticle(Particle.FLAME, pillar, 1, 0.05, 0, 0.05, 0.02);
                        }
                    }
                    world.playSound(center, Sound.ENTITY_FIREWORK_ROCKET_TWINKLE, 1.4f, 0.5f + (float)(waveRadius / 20.0));
                }
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                double spread = 0.8 - tick * 0.025;
                for (int i = 0; i < 12; i++) {
                    double a = i * (Math.PI * 2.0 / 12) + tick * 0.3;
                    Location g = center.clone().add(Math.cos(a) * spread, tick * 0.6, Math.sin(a) * spread);
                    world.spawnParticle(Particle.FLAME, g, 2, 0.1, 0.05, 0.1, 0.05);
                    world.spawnParticle(Particle.SOUL_FIRE_FLAME, g, 1, 0.05, 0.05, 0.05, 0.02);
                }
                // Lava sprays
                if (tick % 4 == 0) {
                    world.spawnParticle(Particle.LAVA, center.clone().add(0, tick * 0.4, 0), 4, 0.3, 0.1, 0.3, 0);
                }
                tick++;
            }
        }.runTaskTimer(plugin, 2L, 1L);

        // Phoenix wing silhouette at the top of the geyser — delayed
        new BukkitRunnable() {
            @Override
            public void run() {
                phoenixWings(center.clone().add(0, 10, 0), world);
            }
        }.runTaskLater(plugin, 28L);
    }
