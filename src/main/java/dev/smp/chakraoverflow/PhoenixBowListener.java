...
public class PhoenixBowListener implements Listener {
    ...
    public void onProjectileShoot(org.bukkit.event.entity.ProjectileLaunchEvent e) {
        ...
        Long startTime = charging.get(player.getUniqueId());
        double chargeSeconds = startTime != null
                ? (System.currentTimeMillis() - startTime) / 1000.0
                : 0;
        double chargeRatio = Math.min(chargeSeconds / MAX_CHARGE_SECONDS, 1.0);

        // Block the shot if charge isn't complete
        if (chargeRatio < 1.0) {
            e.setCancelled(true);
            int bars = (int)(chargeRatio * 10);
            player.sendActionBar(Component.text(
                "§c§lNot ready! §r§6Charging: §c" + "█".repeat(bars) + "§8" + "█".repeat(10 - bars)
            ));
            player.playSound(player.getLocation(), Sound.ENTITY_VILLAGER_NO, 0.8f, 1.2f);
            return;
        }

        // Full charge — consume tracking state
        charging.remove(player.getUniqueId());
    ...
    private void phoenixEruption(Location impactLoc, Player target, Player shooter) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                // ── 5-arm helix — tight, thick, concentrated ──
                for (int arm = 0; arm < ARMS; arm++) {
                    double armOffset = arm * (Math.PI * 2.0 / ARMS);
                    for (double h = 0; h <= HELIX_HEIGHT; h += 0.12) { // 0.12 step = very dense column
                        ...
                        Location loc = origin.clone().add(cx, h, cz);

                        // 3 flame particles per point — fills in the tube solidly
                        world.spawnParticle(Particle.FLAME,           loc, 3, 0.04, 0.04, 0.04, 0);
                        world.spawnParticle(Particle.SOUL_FIRE_FLAME, loc, 2, 0.04, 0.04, 0.04, 0);

                        // Inner lava core on every arm — concentrated glow
                        if (Math.random() < 0.25)
                            world.spawnParticle(Particle.LAVA, loc, 1, 0.02, 0.02, 0.02, 0);

                        // Dense smoke border wrapping each arm
                        if (Math.random() < 0.15)
                            world.spawnParticle(Particle.LARGE_SMOKE, loc, 1, 0.06, 0.06, 0.06, 0);
                    }
                }

                // ── Ground ring — solid band, not just dots ──
                for (int i = 0; i < 96; i++) { // 96 points = no gaps
                    double a    = baseAngle + i * (Math.PI * 2.0 / 96);
                    Location gL = origin.clone().add(Math.cos(a) * radius, -0.4, Math.sin(a) * radius);
                    world.spawnParticle(Particle.FLAME,           gL, 2, 0.03, 0.05, 0.03, 0);
                    world.spawnParticle(Particle.SOUL_FIRE_FLAME, gL, 1, 0.03, 0.03, 0.03, 0);
                }

                // ── Top crown — dense cap ──
                for (int i = 0; i < 48; i++) {
                    double a  = baseAngle * 1.5 + i * (Math.PI * 2.0 / 48);
                    double cr = radius * 0.3;
                    world.spawnParticle(Particle.FLAME, origin.clone().add(Math.cos(a) * cr, HELIX_HEIGHT + 1.0, Math.sin(a) * cr),
                            3, 0.05, 0.08, 0.05, 0);
                    world.spawnParticle(Particle.SOUL_FIRE_FLAME, origin.clone().add(Math.cos(a) * cr, HELIX_HEIGHT + 1.0, Math.sin(a) * cr),
                            2, 0.05, 0.08, 0.05, 0);
                }
    ...
    private void phoenixFinalExplosion(Location center, Player target, Player shooter) {
        ...
        // ── 5 independent horizontal rings, each at a different height, all expand outward simultaneously ──
        // Heights: ground (0.2), knee (1.0), waist (2.0), chest (3.2), head (4.5)
        double[] ringHeights = { 0.2, 1.0, 2.0, 3.2, 4.5 };
        // Stagger each ring's launch by 3 ticks so they don't all start at once
        for (int ri = 0; ri < ringHeights.length; ri++) {
            final double ringY    = ringHeights[ri];
            final int    ringDelay = ri * 3;
            ...
            new BukkitRunnable() {
                ...
                @Override
                public void run() {
                    if (radius > 22) { cancel(); return; }

                    radius += 0.55; // each ring expands 0.55 blocks/tick → reaches 22 in ~40 ticks

                    // Points scale so circumference is always fully covered (~10 pts per block)
                    int points = (int) Math.max(48, radius * 10);
                    ...
                    for (int i = 0; i < points; i++) {
                        ...
                        // Primary ring position
                        Location rim = center.clone().add(cosA * radius, ringY, sinA * radius);

                        // 4 tight particles per point — solid, continuous band
                        world.spawnParticle(Particle.FLAME,           rim, 4, 0.02, 0.04, 0.02, 0);
                        world.spawnParticle(Particle.SOUL_FIRE_FLAME, rim, 3, 0.02, 0.03, 0.02, 0);

                        // Lava pops every 8th point
                        if (i % 8 == 0) world.spawnParticle(Particle.LAVA, rim, 1, 0, 0.15, 0, 0);
                    }

                    // Sound travels with the ring — pitch drops as it gets further away
                    if (tick % 5 == 0) {
                        float pitch = Math.max(0.3f, 1.6f - (float)(radius / 20.0));
                        world.playSound(center, Sound.ENTITY_FIREWORK_ROCKET_TWINKLE, 1.4f, pitch);
                    }

                    tick++;
                }
            }.runTaskTimer(plugin, ringDelay, 1L);
        }

        // ── Central upward geyser burst — dense rising column ──
        new BukkitRunnable() {
            int tick = 0;
            @Override
            public void run() {
                if (tick >= 30) { cancel(); return; }
                double spread = Math.max(0.08, 0.7 - tick * 0.02);
                for (int i = 0; i < 24; i++) {
                    double a = i * (Math.PI * 2.0 / 24) + tick * 0.3;
                    Location g = center.clone().add(Math.cos(a) * spread, tick * 0.55, Math.sin(a) * spread);
                    world.spawnParticle(Particle.FLAME,           g, 3, 0.05, 0.03, 0.05, 0);
                    world.spawnParticle(Particle.SOUL_FIRE_FLAME, g, 2, 0.04, 0.03, 0.04, 0);
                }
                world.spawnParticle(Particle.LAVA, center.clone().add(0, tick * 0.35, 0), 5, 0.2, 0.05, 0.2, 0);
                tick++;
            }
